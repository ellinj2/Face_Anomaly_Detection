...
def main(args):
    ...
    # load model 
    model = ADIFI(load_path=model_path)
    if cuda:
        model.to("cuda:0") 
    ...
    for i in tqdm(range(N_batchs+1)):
        ...
        for image_path, X, y in zip(image_p_batch, X_batch, y_batch):
            if save_images:
                ...
                X_boxed = draw_bounding_boxes(X, y["boxes"], labels=y["labels"], colors=["green", "red"], width=2)
                toPIL(X_boxed).save(os.path.join(out_path, os.path.basename(image_path)))
            
            for bbox, label, score in zip(y["boxes"], y["label"], y["score"]):
